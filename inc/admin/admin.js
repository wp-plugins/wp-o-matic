...
Event.observe( window, 'load', function(){
	...
	if($('edit_tabs')) {
      ...
      if($F(el).length > 0)
      {
        ...
        new Ajax.Request('admin-ajax.php', {
          parameters: $H({ 'action': 'test-feed', 'url': el.value, 'cookie': encodeURIComponent(document.cookie) }),
          onComplete: function(t){
            el.className = (t.responseText == '1') ? 'ok input_text' : 'err input_text';
          }
        });
      }
		};
    ...
    update_feeds();
		...
		Event.observe('quick_add', 'click', function(){
			new Insertion.Bottom('categories', '<li><input type="checkbox" checked="checked" name="campaign_newcat[]" /> <input type="text" name="campaign_newcatname[]" /></li>');
