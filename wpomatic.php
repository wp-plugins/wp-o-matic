...
class WPOMatic {               
  ...
  # Editable options
  # true by default until upgrading is implemented
  var $delete_tables = true;  
  ...
  {              
    ...
    // upgrade feature due for 1.0
//    $this->needs_upgrade = $this->installed ? version_compare(get_option('wpo_version'), $this->version, '<') : false;
    $this->needs_upgrade = false;
    ...
    dbDelta(  "CREATE TABLE `{$this->db['campaign_post']}` (
     ...
     dbDelta(  "CREATE TABLE `{$this->db['log']}` (
      ...
      WPOTools::addOptions(array(
       ...
       'wpo_version'      => array($this->version, 'Installed version log')
      ));
      
   	  $this->installed = true;
    } else if($this->needs_upgrade)
    {
      $this->setup = false;
      
      if($this->tablesExist())
        $this->display_upgrade = true;
      else
      {
  ...
  {   
    ...
    // Delete options
    // We remove wpo_version until upgrading functionality is implemented
    WPOTools::deleteOptions(array('wpo_version', 'wpo_setup', 'wpo_log', 'wpo_log_stdout', 'wpo_unixcron', 'wpo_croncode', 'wpo_cacheimages', 'wpo_cachepath'));
  }                                        
  ...
  function getCommand()
  {
    ...
    return WPOTools::pick($commands[0], $commands[1], $commands[2], $commands[3]);
  }
  ...
  function processItem(&$campaign, &$feed, &$item)
  {
    ...
    // Create post
    $postid = $this->insertPost($wpdb->escape($item->get_title()), $wpdb->escape($content), $date, $categories, $campaign->posttype, $campaign->authorid, $campaign->allowpings, $campaign->allowcomments, $meta);
  ...
  {   
    ...
    extract(WPOTools::getQueryArgs($args, array('fields' => '*',      
  	...
  	if($unparsed)
  	  $where .= " AND (frequency + lastactive) < ". current_time('timestamp', true) . " ";
  ...
  {                 
    ...
    // if user needs to upgrade (TODO: for 1.0)
    //if($this->needs_upgrade)
    //  $this->section = 'upgrade';
    //elseif(! $this->setup)    
    //  $this->section = 'setup';
    //else
    //  $this->section = $_REQUEST['s'] ? $_REQUEST['s'] : $this->sections[0];
  ...
  {  
    ...
    $this->campaign_data['main'] = array(      
        ...
        'allowcomments' => (int) isset($_REQUEST['campaign_allowcomments']),
    ...
    if(! ($this->campaign_data['main']['max'] > 0) )
    {
